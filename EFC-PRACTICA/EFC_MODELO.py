...
listaA = [['001', 'Matemática', 1, 1000, 30, 20], ['002', 'Física', 2, 1500, 25, 15], ['003', 'Química', 3, 2000, 20, 10], ['004', 'Biología', 4, 2500, 15, 5], ['005', 'Historia', 5, 3000, 10, 0]]
listaB = []
...
def cantidad_alumnos(listaB):
    ...
    for item in listaB:
        if 'María' in item[2]:
            cantidad += 1
    print('Cantidad de alumnos con María en su nombre:', cantidad)
...
def mostrarAlumnos(listaB):
    for item in listaB:
        print(item)

def mostrarMaterias(listaA):
    for item in listaA:
        print(item)
